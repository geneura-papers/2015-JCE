...
% [Pedro] no, ese nombre es correcto en este ámbito.
...
una implantación masiva, y además solo recogen información
sobre la cantidad de vehículos que han circulado. Muchas de esas
...
Gracias a que un alto porcentaje de la población posee y utiliza cada vez más dispositivos de comunicación ubicuos, tales
como teléfonos móviles, es posible obtener información sobre la situación y movimiento de dichos individuos basándose en la
detección de estos dispositivos. Esto puede ayudar a gestionar de manera óptima las
...
% No existe ningún registro de la "mac" del dispositivo con su usuario. No es la tarjeta SIM de la que hablamos, sino de la MAC del dispositivo físico.
...
Existen sistemas que han utilizado routers WiFi para posicionamiento. Por ejemplo, en \cite{Rivadeneyra10Wifi} se utiliza la intensidad de hotspots WiFi en un dispositivo móvil para localización en interiores, usando una base de datos de intensidades y posiciones como referencia. 
...
% Antares - la conexión estándar de los cables coaxiales.
para la antena.
...
% El alcance de la comunicación viene determinado por su clase. Clase 1 es la de mayor alcance y se reserva para comunicaciones hasta 100 metros. Clase 2 es la empleada por dispositivos móviles comunes. Clase 3 se reserva a periféricos y werabl.es.
...
% Antares - Son certificaciones estándares, igual habría que ponerlo pero sacado del organismo oficial :S
...
% Antares - cambio lo de que no tiene acceso, porque es el único que solo tiene acceso de ESCRITURA :P
...
% Antares - no lo sé, no me suena bien xD
...
% Antares - todo el sistema operativo se prepara para poder funcionar sin hacer escrituras en disco. Se puede entender de que el sistema de archivos no es el sistema operativo, pero creo que queda más claro así.
...
% Antares - xD
 \textit{usbcore.autosuspend=-1} en el módulo USBCORE del kernel del SO que indica que no puede suspender los dispositivos USB. Normalmente la opción incorpora un intervalo de tiempo que indica cuando tiene que estar sin uso un dispositivo para considerarlo para la suspensión.
...
El software ha sido desarrollado en Java siguiendo un metodología de programación orientada a eventos y multihebrada. A pesar de que la Raspberry Pi Model B empleada en los prototipos descritos dispone de un único procesador, el software ha sido desarrollado teniendo en mente su posterior aplicación a arquitecturas con mayor cantidad de unidades de procesamiento, como la Raspberry Pi 2.
...
% Antares - se pueden capturar "partes" del paquete o paquetes cuyas cabeceras no estén contempladas en el scanner.
...
Ezequiel es el software encargado del acceso a la base datos, consulta y generación de resúmenes de datos, aplicación de algoritmos de predicción y minería de datos y difusión los resultados en la nube \textit{Anan}.
...
% Antares - pues si, pero no se me ocurre que más poner xD Si Fergu quiere poner alguna cita :P
...
En el trabajo se ha descrito en detalle el hardware utilizado, Raspberry Pi Model B, así como los diferentes elementos software necesarios en el dispositivo de monitorización (sistema operativo, software de captación, pasarela de red) y en los servidores adicionales (servidor de entrada de datos y base de datos local). Además, se ha  comentado el servicio de publicación de datos en la nube que se usa en el sistema.

Como trabajo futuro se plantea mejorar el sistema utilizando el modelo más avanzado de Raspberry Pi (modelo 2), que permitirá, principalmente, mejorar el procesamiento del sistema (con paralelización en hebras) y añadir nuevos sensores de captación (GSM, luminosidad, temperatura, etc). 
Además, se planea diseñar y ofrecer distintos algoritmos de predicción, así como la combinación de los datos capturados con otras fuentes de datos abiertas. Así se podrá realizar un estudio sobre la proporción estadística de dispositivos móviles por vehículo, además de pruebas de velocidad y comportamiento en entornos ruidosos o un análisis de consumos de potencia.
